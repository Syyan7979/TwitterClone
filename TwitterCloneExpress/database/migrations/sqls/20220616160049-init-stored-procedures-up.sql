...
BEGIN
SELECT userId, userName, twitterHandle, email, profileImage, headerImage, timeStamp  FROM users;
END;
...
BEGIN
SELECT 
	userId, userName, twitterHandle, email, profileImage, headerImage, timeStamp 
FROM 
	users
WHERE userId = id;
END;
...
INSERT INTO
	users 
(userId, userName, twitterHandle, email, password, profileImage, headerImage, timeStamp) 
VALUES 
...
FROM 
	users
WHERE
	(userName = usrName AND password = pwrd) OR (email = usrEmail AND password = pwrd);
END;
...
BEGIN
DELETE FROM follows WHERE followerId = frId AND followeeId = feId;
END;
